...
const colorsByLength = Object.keys(colors).sort((a, b) => a.length - b.length); //function to determine if the word is a valid color name

exports.colorsByLength = colorsByLength;

function isValidColor(word) {
  return !!colors[word]; // Coerce it to return a true or false instead of the hex with the double bang !!
}
...
function handleResults({
  ...
  logWords(results);
  const words = results[results.length - 1][0].transcript; //see if the word is a valid color
  //lowercase everything

  let color = words.toLowerCase(); //strip any spaces out

  color = color.replace(/\s/g, ""); //if valid color

  if (!(0, _colors.isValidColor)(color)) return; //show UI
...
function start() {
  ...
  recognition.onresult = _handlers.handleResults; // listens for a result

  recognition.start();
}

start(); // write a function to map through each color and return its name
